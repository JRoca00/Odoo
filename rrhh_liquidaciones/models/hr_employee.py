...
class HrEmployee(models.Model): 
    ...
    def update_codigo_empleado(self):
        ...
        #CODIGO PARA CREARLE EL USUARIO AL EMPLEADO
        #def user_employee(self):
        for record in self:
            user_vals = {
                'name': record.name,
                'login': record.barcode,
                'password': record.barcode,
                'groups_id': [(6, 0, [self.env.ref('base.group_user').id])] 
            }


            existing_user = self.env['res.users'].search([('login', '=', record.barcode)], limit=1)
            if existing_user:
...
#Versiones posteriores a la 14 no tienen en el campo code
...
class HrEmployeeVac(models.Model):
    ...
    @api.depends('vac_list','type_vac')
    def _compute_vac(self):
        for record in self:
            record.vac = sum(ausencia.number_of_days for ausencia in record.vac_list)
            record.vacg = sum(ausencia.number_of_days for ausencia in record.vac_list)
            salario = self.employeeid.contract_ids.wage
            if record.type_vac == 'Gozadas':
                record.tot_vac = 0
            else:
